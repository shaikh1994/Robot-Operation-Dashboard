...
import notificationsData from '../data/mockData.json';
...
function DropdownNotifications({
  ...
  // Load notifications from the imported data when the component mounts
  useEffect(() => {
  ...
  return (
    <div className="relative inline-flex">
      ...
      >
        ...
        <li key={notification.id} className="border-b border-slate-200 dark:border-slate-700 last:border-0">
          <Link
            className="block py-2 px-4 hover:bg-slate-50 dark:hover:bg-slate-700/20"
            to="#0"
            onClick={() => setDropdownOpen(!dropdownOpen)}
          >
            <span className="block text-sm mb-2">{notification.content}</span>
            <span className="block text-xs font-medium text-slate-400 dark:text-slate-500">Date: {notification.date}</span>
          </Link>
        </li>
      ))}
          </ul>
